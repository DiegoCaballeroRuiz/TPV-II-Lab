...
void server(Uint16 port) {
	...
	char buffer[256];
	int result = 0;
	memset(buffer, 0, 256); // reset the buffer to 0 everywhere

	// a variable that represents the IP address -- in this case only the port
	IPaddress ip;
	...
	// We want to use non-blocking communication, and the way to do this is by using a socket set.
	...
	SDLNet_SocketSet socketSet = SDLNet_AllocSocketSet(1); // we will add only 1 socket to the set
	...
	while (!done) {
		...
		if (SDLNet_CheckSockets(socketSet, 60000) > 0) {
			...
			// before calling SDLNet_SocketReady we must have called SDLNet_CheckSockets, they
			// work together: SDLNet_CheckSockets marks those with activity as read, and SDLNet_SocketReady
			// clears that mark.
...
void client(const char *host, Uint16 port) {
	...
	// - wait for the server's response (recall that SDLNet_TCP_Recv blocks until it receives something)
