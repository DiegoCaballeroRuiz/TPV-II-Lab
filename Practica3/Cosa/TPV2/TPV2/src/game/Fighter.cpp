...
void Fighter::update() {
	...
	Game::Instance()->getNetworking().send_state(p.pos, p.width, p.height,
			p.rot);
...
void Fighter::shoot(Player &p) {
	if (_ihdlr.keyDownEvent() && _ihdlr.isKeyDown(SDL_SCANCODE_SPACE)) {
		...
		Game::Instance()->getNetworking().send_shoot(bp, bv, bwidth, bheight,
...
void Fighter::send_my_info() {
	...
	Game::Instance()->getNetworking().send_my_info(p.pos, p.width, p.height,
			p.rot, p.state);
}
