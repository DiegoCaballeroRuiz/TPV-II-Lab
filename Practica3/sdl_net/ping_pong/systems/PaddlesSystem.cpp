...
void PaddlesSystem::initSystem() {
	auto ballEntity = _mngr->getHandler(ecs::hdlr::_hdlr_BALL);
	_ballTr = _mngr->getComponent<Transform>(ballEntity);
	assert(_ballTr != nullptr);

	// create the left paddle
	auto leftPaddle = _mngr->addEntity(ecs::grp::_grp_PADDLES);
...
//	leftPaddleCtrl->setType(PaddleCtrl::AI);
	...
	auto rightPaddle = _mngr->addEntity(ecs::grp::_grp_PADDLES);
...
//	rightaddleCtrl->setType(PaddleCtrl::AI);
	...
	_paddles[0] = leftPaddle;
	_paddles[1] = rightPaddle;

}

void PaddlesSystem::update() {
//	for (auto e : _mngr->getEntities(ecs::_grp_PADDLES)) {
//		movePaddle(e);
//	}

	auto netSys = _mngr->getSystem<NetworkSystem>();

	auto side = netSys->getSide();
	movePaddle(_paddles[side]);
	auto tr = _mngr->getComponent<Transform>(_paddles[side]);
