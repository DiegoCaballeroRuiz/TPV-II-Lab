...
//   a,b,c and array x of 10 elements.
//
// _IMPL_SERIALIAZION_B(T,a,b,c,x,10u,...)
//
//   define method serialize and deserialize for attributes,
//   a,b,c and array x of 10 elements. But they will be preceded
//   by a call to T::serialize(buf) and T::deserialize(buf) respectively.
//   This should be used in derived structs to serialize base structs
//   first
//
...
private:
	...
	inline static Uint8* serialize_uint(UnsignedT &v, Uint8 *buf) {
		static_assert(std::is_unsigned_v<UnsignedT> && std::is_integral_v<UnsignedT>);
	...
	inline static Uint8* serialize_sint(SignedT &v, Uint8 *buf) {
		static_assert(std::is_unsigned_v<UnsignedT> && std::is_integral_v<UnsignedT>);
		static_assert(std::is_signed_v<SignedT> && std::is_integral_v<SignedT>);
