...
void Game::init() {
	...
	ImageRenderer* renderer = new ImageRenderer(&sdlutils().images().at("fighter"));
	_fighter->addComponent(renderer);

	FighterCtrl* ctrl = new FighterCtrl();
	_fighter->addComponent(ctrl);

	SimpleMove* simpleMove = new SimpleMove();
	_fighter->addComponent(simpleMove);

	DeAcceleration* deacceleration = new DeAcceleration();
	_fighter->addComponent(deacceleration);

	ShowAtOppositeSide* toroide = new ShowAtOppositeSide();
	_fighter->addComponent(toroide);
